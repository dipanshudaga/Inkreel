...
export default async function WatchDiary() {
  // Fetch movies, anime, and tv
  ...
  const totalLogged = watchItems.filter(i => i.status === 'completed').length;
  const thisYear = watchItems.filter(i => i.status === 'completed' && (i.completedAt?.startsWith('2026') || false)).length;
      ...
      <div className="flex-1 flex flex-col border-r border-[#1A1A1A]">
        <div className="flex flex-col pt-8 pb-4 gap-6 border-b border-[#1A1A1A] px-8">
          ...
          <div className="flex gap-4">
            ...
            <FilterButton label="TV Shows" />
            <FilterButton label="Anime" />
          </div>
        </div>
        ...
        <div className="flex flex-col py-8 px-6 gap-12 sticky top-0">
          ...
          <div className="flex flex-col gap-4">
            ...
            {activeItems.map((item) => (
              <Link key={item.id} href={`/items/${item.id}`} className="flex gap-4 bg-white border border-[#1A1A1A] p-3 hover:bg-traced-surface transition-colors cursor-pointer group">
                ...
                <div className="flex flex-col justify-between py-0.5">
                  <div className="flex flex-col gap-1">
                    <div className="uppercase tracking-[0.05em] text-traced-accent font-sans text-[11px] font-semibold">
                      {item.type === 'movie' ? 'Watching' : 'Consuming'}
                    </div>
                    <div className="text-[18px] leading-tight text-traced-dark font-serif font-medium line-clamp-1 group-hover:text-traced-accent transition-colors">
                      {item.title}
                    </div>
                  </div>
                  <div className="text-[#737373] font-sans text-xs uppercase tracking-wider">
