...
export default async function ReadDiary({ searchParams }: ReadDiaryProps) {
  ...
  // Group by year
  const byYear = readItems.reduce<Record<string, typeof readItems>>((acc, item) => {
  ...
  const filters = [
    ...
    { label: "Reading List", value: "watchlist" },
        ...
        <div className="flex flex-col overflow-y-auto">
          {readItems.length === 0 && (
            <div className="w-full py-32 text-center flex flex-col items-center gap-4 px-8">
              <p className="text-[#A1A19A] font-serif italic text-2xl">Your read diary is empty.</p>
              <p className="text-[#737373] font-sans text-xs uppercase tracking-widest">Use search to find a book and add it.</p>
            </div>
          )}
